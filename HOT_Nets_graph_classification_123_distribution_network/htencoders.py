...
class CNN(nn.Module):
    def __init__(self, dim_out):
        ...
        self.features = nn.Sequential(
            nn.Conv2d(1, 8, kernel_size=2, stride=2), #channel of PI is 1
    ...
    def forward(self, PI):
        PI = torch.unsqueeze(PI, dim = 1)
        feature = self.features(PI) #channel of PI is 1
...
class GcnEncoderGraph(nn.Module):
    def __init__(self, input_dim, edge_input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
        ...
        self.tranformation_weight = nn.Parameter(torch.FloatTensor(args.max_nodes, args.max_edges))
        nn.init.kaiming_uniform_(self.tranformation_weight, mode='fan_out', a=math.sqrt(5))

        # attention mechanism
        self.attention = Attention(hidden_dim * (num_layers - 1) + embedding_dim)

        # DNN for PI
        self.degree_CNN = CNN(hidden_dim * (num_layers - 1) + embedding_dim)
        self.betweenness_CNN = CNN(hidden_dim * (num_layers - 1) + embedding_dim)
        self.closeness_CNN = CNN(hidden_dim * (num_layers - 1) + embedding_dim)
        ...
        # for higher-order simplicies-level
        self.hodge_conv_first, self.hodge_conv_block, self.hodge_conv_last = self.build_hodge_conv_layers(
            edge_input_dim, edge_hidden_dim, edge_embedding_dim, num_layers,
            add_self, normalize=True, dropout=dropout)
        ...
        # weight initialization for all frameworks
        for m in self.modules():
            if isinstance(m, GraphConv):
                m.weight.data = init.xavier_uniform(m.weight.data, gain=nn.init.calculate_gain('relu'))
                if m.bias is not None:
                    m.bias.data = init.constant(m.bias.data, 0.0)
            elif isinstance(m, HodgeConv):
                m.weight.data = init.xavier_uniform(m.weight.data, gain=nn.init.calculate_gain('relu'))
                if m.bias is not None:
                    m.bias.data = init.constant(m.bias.data, 0.0)
    ...
    def build_hodge_conv_layers(self, input_dim, hidden_dim, embedding_dim, num_layers, add_self,
            normalize=False, dropout=0.0):
        conv_first = HodgeConv(input_dim=input_dim, output_dim=hidden_dim, add_self=add_self,
                normalize_embedding=normalize, bias=self.bias)
        conv_block = nn.ModuleList(
                [HodgeConv(input_dim=hidden_dim, output_dim=hidden_dim, add_self=add_self,
                        normalize_embedding=normalize, dropout=dropout, bias=self.bias)
                 for i in range(num_layers-2)])
        conv_last = HodgeConv(input_dim=hidden_dim, output_dim=embedding_dim, add_self=add_self,
                normalize_embedding=normalize, bias=self.bias)
        return conv_first, conv_block, conv_last
    ...
    def gcn_forward(self, x, adj, conv_first, conv_block, conv_last, embedding_mask=None): # gcn_forward is used for GcnSet2SetEncoder and SoftPoolingGcnEncoder
    ...
    def forward(self, x, edge_x, adj, edge_adj, degree_PI, betweenness_PI, closeness_PI, batch_num_nodes=None, **kwargs):
        # x: (B, max_nodes, num_node_feature)
        # edge_x: (B, max_edges, num_edge_feature)
        # adj: (B, max_nodes, max_nodes)
        # edge_adj: (B, max_edges, max_edges)
        # mask
        ...
        if self.bn:
            x = self.apply_bn(x)
        out_all = []
        node_out, _ = torch.max(x, dim=1)
        out_all.append(node_out)
        ...
        if self.num_aggs == 2:
            ...
            out_all.append(node_out)
        if self.concat:
            node_output = torch.cat(out_all, dim=1)
        else:
            node_output = node_out

        # *********************************
        # conv in higher-order simplicies-level
        edge_x = self.hodge_conv_first(edge_x, edge_adj)
        edge_x = self.act(edge_x)
        if self.bn:
            edge_x = self.apply_bn(edge_x)
        edge_out_all = []
        edge_out, _ = torch.max(edge_x, dim=1)
        edge_out_all.append(edge_out)
        for i in range(self.num_layers - 2):
            edge_x = self.hodge_conv_block[i](edge_x, edge_adj)
            edge_x = self.act(edge_x)
            if self.bn:
                edge_x = self.apply_bn(edge_x)
            edge_out, _ = torch.max(edge_x, dim=1)
            edge_out_all.append(edge_out)
            if self.num_aggs == 2:
                edge_out = torch.sum(edge_x, dim=1)
                edge_out_all.append(edge_out)
        edge_x = self.hodge_conv_last(edge_x, edge_adj)
        # edge_x = self.act(edge_x)
        edge_out, _ = torch.max(edge_x, dim=1)
        edge_out_all.append(edge_out)
        if self.num_aggs == 2:
            ...
            edge_out_all.append(edge_out)
        if self.concat:
            edge_output = torch.cat(edge_out_all, dim=1)
        else:
            edge_output = edge_out

        # DNN on PIs
        degree_topo_output = self.degree_CNN(degree_PI)
        betweenness_topo_output = self.betweenness_CNN(betweenness_PI)
        closeness_topo_output = self.closeness_CNN(closeness_PI)

        # attention mechanism on node-, edge-, and topo-embeddings
        emb_output = torch.stack([edge_output, degree_topo_output, betweenness_topo_output, closeness_topo_output], dim=1)
        emb_output, att = self.attention(emb_output)

        ypred = self.pred_model(emb_output)
...
class SoftPoolingGcnEncoder(GcnEncoderGraph):
    ...
    def forward(self, x, adj, batch_num_nodes, **kwargs):
        if 'assign_x' in kwargs: ### where assign_x is used!
            x_a = kwargs['assign_x']
        else:
            x_a = x
